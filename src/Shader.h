...
public:
    ...
    Shader(const char* vertexPath, const char* fragmentPath) { // uses stupid absolute linking
        std::string vertexCode, fragmentCode;
        std::ifstream vShaderFile, fShaderFile;
        // check if shader files are capable for exceptions
        ...
        checkCompileErrors(vertex, "VERTEX"); // calling private defined function to catch for errors
        ...
        ID = glCreateProgram(); // create then load shaders into program
        glAttachShader(ID, vertex);
        glAttachShader(ID, fragment);
        glLinkProgram(ID);
        checkCompileErrors(ID, "PROGRAM");

        glDeleteShader(vertex); // delete shaders as they've been loaded into the program
...
private:
    // utility method to check for compiling errors
